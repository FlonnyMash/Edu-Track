...
function buildReviewBlock(progress: SyllabusProgress): string {
  return [
    ...
    "Interactive SRS flashcards (Got it / Missed it) are handled separately by the app —",
    "your # SIDE QUEST must contain written study instructions only (offline drills, writing, recall).",
...
export function getDailyTaskPrompt(
  ...
  return [
    ...
    reviewBlock,
    "",
    "── SYLLABUS SYNC (AI vocabulary suggestions) ──",
    "Analyze the user's progress and today's assigned topic.",
    "In Part 2 JSON, populate newly_suggested_items with 1–5 level-appropriate NEW items",
    "(vocabulary or kana) the user should consider adding to their personal learning loop.",
    "Each suggestion must include term, meaning (English), and romanji.",
    "Skip items already taught in today's MAIN QUEST or listed in SIDE QUEST review.",
    "Use [] when no new suggestions are appropriate.",
    "",
    handwritingBlock,
    "",
    outputFormatBlock,
    ...
    "Quest categorization (required for the app UI):",
    "  • # MAIN QUEST = today's primary learning goal (new material).",
    "  • # SIDE QUEST = bonus review or preparation instructions (written drills only).",
    "  • Interactive SRS reviews are NOT part of Part 1 — the app surfaces those separately.",
    ...
    '{ "topic": "...", "chapter": "...", "next_recommended_action": "advance|stay|review", "estimated_duration": 30, "newly_suggested_items": [] }',
    ...
    "- newly_suggested_items: array of 0–5 NEW vocabulary or kana items appropriate for the user's level",
    '  Each object: { "term": "...", "meaning": "English gloss", "romanji": "romaji reading" }',
    "  Suggest items aligned with today's topic and user progress (e.g. next Genki vocabulary).",
    "  Do NOT repeat characters/vocabulary already in today's MAIN QUEST items or SIDE QUEST review list.",
    "  Use an empty array [] when nothing new should be suggested.",
    "",
    "── OUTPUT RULES ──",
